...
public class Main {
    ...
    private static int tsp(int city, int visitBitmask) {
        ...
        if(visitBitmask == (1 << n) - 1) {	// 모든 도시를 방문했다면
            if(w[city][0] == 0) {	// 이런 경우는 거의 없겠지만, 혹시 발생하는 경우 예외 처리
                return INF;
            }

            return w[city][0];	// 현재 도시 -> 0번쨰(시작) 도시 이동 거리
        }

        if(dp[city][visitBitmask] != -1) {	// dp값이 존재하는경우
        ...
        for(int i = 0; i < n; i++) {	// 현재 도시(city)에서 각 i의 도시로 이동한 경우에 대해 DFS 수행
            int next = visitBitmask | (1 << i);

            if((visitBitmask & (1 << i)) != 0 || w[city][i] == 0) continue;
            ...
            dp[city][visitBitmask] = Math.min(dp[city][visitBitmask], tsp(i, next) + w[city][i]);	// dfs(다음 도시, 다음도시 방문했다고 가정) + 여기서 다음 도시까지의 거리 와 최소거리 비교
        }
